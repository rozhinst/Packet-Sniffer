...
def capture(conn):
    ...
    while True:
        ...
        if eth_proto == 8:
            ...
            hardware_type, proto_type, sender_h_addr, sender_proto_addr, target_h_addr, target_proto_addr = arp_packet(
                data)
            print('\t - ARP')
            print(
                '\t\t - Hardware Type: {}, Protocol Type: {}'.format(hardware_type, proto_type))
            print('\t\t - Sender Hardware Address: {}, Sender Protocol Address: {}'.format(
                sender_h_addr, sender_proto_addr))
            print('\t\t - Target Hardwar Address: {}, Target Protocol Address: {}'.format(
                target_h_addr, target_proto_addr))
        else:
            print('Data: ')
            print(format_string('\t ', data))
...
def ipv4_packet(data):
    ...
    total_length, id, flags_offset, ttl, proto, src, target = struct.unpack(
        '! 2x 2s 2s 2s B B 2x 4s 4s', data[:20])
...
def arp_packet(data):
    ...
    arp_count += 1
    hardware_type, proto_type, sender_h_addr, sender_proto_addr, target_h_addr, target_proto_addr = struct.unpack(
        '! 2s 2s 4x 4s 4s 4s 4s ', data)

    return hardware_type, proto_type, get_mac_addr(sender_h_addr), ipv4(sender_proto_addr), get_mac_addr(target_h_addr), ipv4(target_proto_addr)
...
def show_graf(total_size):
    sizes = [tcp_count/total_size * 100, udp_count/total_size * 100,
             icmp_count/total_size * 100, arp_count/total_size * 100]
